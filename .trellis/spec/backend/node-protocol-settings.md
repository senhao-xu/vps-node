...
| `ipv6_enabled` true without `ipv6_address`, or `ipv6_address` over 255 characters | `422 validation` |
...
- `nodes.ipv6_enabled` (INTEGER 0/1) + `nodes.ipv6_address` (TEXT) are a **client-facing** pair: `ipv6_address` mirrors `address` (IPv6 literal or domain, 1-255 chars, never resolved by the panel), and they only affect subscription output, never the sing-box inbound, so no agent/kernel change is involved and traffic stays merged per node (`Pair{UserID, NodeID}`); visit logs distinguish families via `client_ip`.
...
## 10. Chain Exit Rendering (singbox-render-v2)

- `nodes.chain_node_id` (nullable self-FK) makes a node an **entry** whose inbound traffic is forwarded through a protocol-client outbound to the referenced **exit** node (any server). `Render(appKey, nodes, chains...)` emits one outbound `tag: chain-<entryNodeID>` plus `route.rules: [{inbound:[<entry tag>], outbound:"chain-<id>"}]`; `route.final` stays `direct`, and with no chains the payload is byte-equivalent to v1.
- Relay credentials are **derived, never stored**: exit-side inbounds carry one pseudo user `relay-<entry server id>` per (exit node, entry server) pair; the matching client credential is `DeriveRelaySSPassword` / `DeriveRelayUUID` / `DeriveRelayPassword`. One name per entry server means multiple entries into the same exit never overwrite each other.
- Relay users are deliberately absent from the config's `users` payload, so the agent `ConnectionTracker` (`Table.userID`) never maps them and relay traffic is neither attributed nor reported; entry-side per-user traffic/device counting is unchanged.
- Reality exit: derive the client public key with `singbox.RealityPublicKey` (x25519 stdlib). Shadowsocks 2022 exit: client password must be `CombineSSClientPassword(cipher, serverKey, userKey)`. Hysteria2/AnyTLS exit: pin the stored certificate when present, else set `tls.insecure`.
- A chain change touches **both** ends: `CreateNodeAndBump` / `UpdateNodeAndBump` / `DeleteNodeAndBump` must bump the owning server, the old and new exit servers, and every server whose nodes chain through the changed node; `DeleteServerCascade` unlinks external entries and bumps them. Cycle detection (`ValidateNodeChain`, self + any depth) is authoritative in the repo; the handler maps it to `422 validation`. Deleting a referenced exit node returns `409 conflict` naming the entries.
