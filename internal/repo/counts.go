...
func (r *Repo) CountOnlineUsersByServerIDs(ctx context.Context, serverIDs []int64) (map[int64]int64, error) {
	if len(serverIDs) == 0 {
		return map[int64]int64{}, nil
	}
	query := `SELECT server_id, COUNT(DISTINCT user_id) FROM online_devices WHERE server_id IN (` + inPlaceholders(len(serverIDs)) + `) GROUP BY server_id`
	return r.groupedCount(ctx, query, int64Args(serverIDs))
}
...
func (r *Repo) CountOnlineUsersByNodeIDs(ctx context.Context, nodeIDs []int64) (map[int64]int64, error) {
	if len(nodeIDs) == 0 {
		return map[int64]int64{}, nil
	}
	query := `SELECT node_id, COUNT(DISTINCT user_id) FROM online_devices WHERE node_id IN (` + inPlaceholders(len(nodeIDs)) + `) GROUP BY node_id`
	return r.groupedCount(ctx, query, int64Args(nodeIDs))
}
