...
func (r *Repo) DeviceBatchExists(ctx context.Context, agentID, seq int64) (bool, error) {
	return r.batchExists(ctx, `SELECT COUNT(*) FROM device_batches WHERE agent_id = ? AND seq = ?`, agentID, seq)
}

func (r *Repo) RecordDeviceBatch(ctx context.Context, agentID, seq int64) error {
	return r.recordBatch(ctx, `INSERT INTO device_batches (agent_id, seq, received_at) VALUES (?, ?, ?)`, agentID, seq)
}
