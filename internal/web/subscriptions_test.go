...
func TestSubscriptionEligibilityAndNodeFiltering(t *testing.T) {
	...
	node, err := e.repo.CreateNode(context.Background(), repo.NewNode{ServerID: server, Name: "active", Protocol: repo.ProtocolShadowsocks, Port: 443, Settings: `{"method":"2022-blake3-aes-128-gcm"}`})
	if err != nil {
		t.Fatal(err)
	}
	disabledServerNode, err := e.repo.CreateNode(context.Background(), repo.NewNode{ServerID: disabledServer, Name: "disabled-server-node", Protocol: repo.ProtocolShadowsocks, Port: 445, Settings: `{"method":"2022-blake3-aes-128-gcm"}`})
	if err != nil {
		t.Fatal(err)
	}
	...
	disabledNode, err := e.repo.CreateNode(context.Background(), repo.NewNode{ServerID: server, Name: "disabled", Protocol: repo.ProtocolShadowsocks, Port: 444, Settings: `{"method":"2022-blake3-aes-128-gcm"}`})
	if err != nil {
		t.Fatal(err)
	}
